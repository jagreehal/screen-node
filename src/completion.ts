// Shell completion. `screen completion <shell>` prints a script the user sources (or installs
// via `screen path`). Kept as pure string-builders so the exact output is unit-testable — the CLI
...
/**
 * Screen-native subcommands plus the expert models and the package-manager front-ends you'd put
...
function zsh(inline: boolean): string {
  const header = inline
    ? '# screen shell completion (zsh)'
    : '#compdef screen screen-node\n# screen shell completion (zsh), generated by `screen completion zsh`';
  const register = inline
    ? '(( $+functions[compdef] )) && compdef _screen screen screen-node'
    : 'compdef _screen screen screen-node';
  return `${header}
_screen() {
  ...
  case $state in
    command)
      _describe 'command' _screen_commands ;;
    args)
...
function bash(): string {
  return `# screen shell completion (bash), generated by \`screen completion bash\`
_screen() {
...
complete -F _screen screen screen-node
`;
}

function fish(): string {
  const lines = ['# screen shell completion (fish), generated by `screen completion fish`'];
  for (const bin of ['screen', 'screen-node']) {
    lines.push(
